...
namespace HappyFarmProjectAPI.Controllers
{
    public class RegionRepository
    {
        ...
        public void EditRegion(int id, EditRegionRequests regionRequest)
        {
            using (HappyFarmPRG4Entities db = new HappyFarmPRG4Entities())
            {
                var region = db.Regions.Where(x => x.Id == id).FirstOrDefault();
        ...
        /// </summary>
        /// <param name="regionRequest"></param>
        public void AddRegion(AddRegionRequests regionRequest)
        {
            using (HappyFarmPRG4Entities db = new HappyFarmPRG4Entities())
            {
        ...
        /// <returns></returns>
        public ResponsePagingModel<List<Region>> GetRegions(int currentPage, int limitPage, string search, string role)
        {
            using (HappyFarmPRG4Entities db = new HappyFarmPRG4Entities())
            {
                // get employees
